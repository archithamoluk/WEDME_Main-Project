...
else
{
	...
	if($amount==300)
	{
		...
		<ul id="flexiselDemo3">
								<li>
								<?php
							 	 $o="SELECT * FROM tbl_users LEFT JOIN profile on tbl_users.id=profile.profile_id 
								where profile.status is null order by tbl_users.id desc limit 0,10";
								$ret=select($o);
	...
	?>
		...
		<ul id="flexiselDemo3">
								...
								<?php
							 	$o="SELECT * FROM tbl_users LEFT JOIN profile on tbl_users.id=profile.profile_id 
								where profile.status is null order by tbl_users.id desc limit 0,50";
								$ret=select($o);
								while($t=mysqli_fetch_array($ret))
								{extract($t);
								?>
	...
	?>
		...
		<ul id="flexiselDemo3">
								...
								<?php
							 	$o="SELECT * FROM tbl_users LEFT JOIN profile on tbl_users.id=profile.profile_id 
								where profile.status is null order by tbl_users.id desc limit 0,100";
								$ret=select($o);
								while($t=mysqli_fetch_array($ret))
								{extract($t);
								?>
